...
interface DirectorNotificationProps {
  id: string;
  requestedBy: string;
  farmLocation: string;
  category: string;
...
export const DirectorNotificationEmail: React.FC<Readonly<DirectorNotificationProps>> = ({
  id,
  requestedBy,
  farmLocation,
  category,
  ...
  <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: '600px', margin: '0 auto', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '32px', backgroundColor: '#ffffff' }}>
    ...
    <h1 style={{ color: '#aa272f', marginTop: 0, fontSize: '24px', borderBottom: '2px solid #f3f4f6', paddingBottom: '16px' }}>Final Approval Required</h1>
    <p style={{ color: '#505050', fontSize: '16px', lineHeight: '1.5' }}>
      A supply request by <strong>{requestedBy}</strong> for <strong>{farmLocation}</strong> is pending your final approval.
    </p>
    ...
    <div style={{ backgroundColor: '#f9fafb', padding: '20px', borderRadius: '8px', marginBottom: '32px', marginTop: '24px', border: '1px solid #e5e7eb' }}>
      ...
      {managerComment && (
        <div style={{ marginTop: '16px', borderTop: '1px solid #e5e7eb', paddingTop: '16px' }}>
          <strong style={{ color: '#374151', fontSize: '15px' }}>Manager's Comment:</strong>
          <p style={{ margin: '8px 0 0 0', fontSize: '14px', fontStyle: 'italic', color: '#6b7280' }}>
            "{managerComment}"
          </p>
        </div>
      )}
    ...
    <div style={{ textAlign: 'center' }}>
      <a 
        href={`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/director/review/${id}`} 
