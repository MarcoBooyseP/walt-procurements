...
export const ManagerNotificationEmail: React.FC<Readonly<ManagerNotificationProps>> = ({
  id,
  requestedBy,
  farmLocation,
  category,
  ...
  <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: '600px', margin: '0 auto', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '32px', backgroundColor: '#ffffff' }}>
    ...
    <h1 style={{ color: '#aa272f', marginTop: 0, fontSize: '24px', borderBottom: '2px solid #f3f4f6', paddingBottom: '16px' }}>New Supply Request</h1>
    <p style={{ color: '#505050', fontSize: '16px', lineHeight: '1.5' }}>
      <strong>{requestedBy}</strong> has submitted a new supply request for <strong>{farmLocation}</strong>.
    </p>
    ...
    <div style={{ textAlign: 'center' }}>
      <a 
        href={`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/manager/review/${id}`} 
